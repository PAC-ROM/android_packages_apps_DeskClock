 ...
 */
public class AnalogPACAppWidgetProvider extends AppWidgetProvider {
    private static final boolean DEBUG = false;

    static final String TAG = "AnalogPACAppWidgetProvider";
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        clockStyleDrawables = context.getResources().getStringArray(R.array.pac_clock_backgrounds_drawable);

        for (int i=0; i<N; i++) {
            int selectedPos = AnalogPACAppWidgetConfigure.loadClockPref(context, appWidgetIds[i]);
    ...
    public void onDeleted(Context context, int[] appWidgetIds) {
        ...
        final int N = appWidgetIds.length;
        for (int i=0; i<N; i++) {
            AnalogPACAppWidgetConfigure.deleteClockPref(context, appWidgetIds[i]);
        }
    }
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(),
            R.layout.analog_pac_appwidget);

        views.setOnClickPendingIntent(R.id.analog_pac_appwidget,
